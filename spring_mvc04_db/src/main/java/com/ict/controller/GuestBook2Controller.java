...
public class GuestBook2Controller {
	...
	public ModelAndView getGuestBook2Insert(GuestBook2VO g2vo, HttpServletRequest request) {
		...
		try {
			...
			if (f_param.isEmpty()) {
				...
				// DB에 저장할 이름을 변경하자
				UUID uuid = UUID.randomUUID(); // 랜덤으로 겹치지 않는 글자로 나온다.
	...
	public void getGuestBook2Down(String f_name, HttpServletRequest request, HttpServletResponse response) {
	...
	public ModelAndView getGuestBook2Update(GuestBook2VO g2vo, HttpServletRequest request) {
		...
		// 비밀번호가 맞는지 틀린지 검사하기. (비밀번호가 암호로 되어있다.)
		// jsp에서 암호로 입력한 것
		String cpwd = g2vo.getPwd(); // 암호화 되지 않은 것
	...
	public ModelAndView getGuestBook2Delete(GuestBook2VO g2vo, HttpServletRequest request) {
		ModelAndView mv = new ModelAndView();
		// jsp에서 암호로 입력한 것
		String cpwd = g2vo.getPwd(); // 암호화 되지 않은 것
		System.out.println("cpwd(delete.jsp에서 넘어온거): " + cpwd);
		...
		if (!passwordEncoder.matches(cpwd, dpwd)) {
			...
			mv.setViewName("redirect:/guestbook2_list.do");
			return mv;
		}
	}
}